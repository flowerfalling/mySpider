...
class NoveletteSpiderSpider(scrapy.Spider):
    ...
    allowed_domains = ['www.86wx.cc']

    def start_requests(self):
        for i in range(1):
            # print(f'page: {i + 1}')
            yield Request(url=f'https://www.86wx.cc/11781/', callback=self.parse,
                          cb_kwargs={'page': i})

    def parse(self, response: HtmlResponse, **kwargs):
        sel = Selector(response)
        li_list = sel.css('#play_0 > ul > li > a')
    ...
    def parse_text(self, response: HtmlResponse, **kwargs):
        item = kwargs['item']
        sel = Selector(response)
        for p in sel.css('#content > p::text')[1:]:
